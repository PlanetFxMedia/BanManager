...
public class GlobalTempBanCommandListener extends Command {

	public GlobalTempBanCommandListener(String name) {
		super(name);
	}
	
	private void sendVersuch(ProxiedPlayer pp) {
		if (pp.hasPermission("pfx.banmanager.gtban")) {
	...
	public void execute(CommandSender cs, String[] args) {
		if (cs instanceof ProxiedPlayer) {
			...
			if (args.length > 0 && pp.hasPermission("pfx.banmanager.gtban")) {
				if (args[0].equalsIgnoreCase("y")) {						
					...
					if (uuiddb != null) {
						...
						if (!BanManager.isPlayerBanned(PlayerUUID)) {
							...
							Ban ban = new Ban(PlayerUUID, PlayerName, BanTime, BannedTime, reason, BannedFromPlayerUUID, BannedFromPlayerName, "GLOBAL");
